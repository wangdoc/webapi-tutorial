...
一般来说，脚本不应该改动用户的剪贴板，以免产生预期以外的结果。但是，有些时候这样做确实能够带来方便，比如“一键复制”功能，用户点击一下按钮，指定的内容就自动进入剪贴板。
...
注意，复制操作最好放在将在事件监听函数里面，由用户触发（比如用户点击按钮）。如果脚本自主执行，某些浏览器可能会报错。
...
首先，它只能将选中的内容，复制到剪贴板，无法向剪贴板任意写入内容。
...
它的所有操作都是异步的，返回 Promise 对象，不会造成页面卡顿。而且，它可以指定任意内容（比如图片）放入剪贴板。

通过`navigator.clipboard`属性，可以获取 Clipboard 对象，所有操作都通过这个对象进行。

```javascript
const clipboardObj = navigator.clipboard;
```

上面示例中，如果不存在`navigator.clipboard`属性，返回`undefined`，就说明当前浏览器不支持这个 API。
